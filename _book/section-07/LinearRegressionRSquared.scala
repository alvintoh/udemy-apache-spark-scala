...
object LinearRegressionRSquared {
  ...
  def main(args: Array[String]) {
    ...
    // Now I have change the step size to be 0.01 in order to reflect a smaller step size in order to find a more accurate optima
    ...
    // Finds the mean of real value
    val mean = predictionAndLabel.map(x => x._2).mean()
    
    // Computes the distance from the mean to the real values and sum them up
    val meanDistanceSquared = predictionAndLabel.map(x => (mean - x._2) * (mean - x._2)).sum()
    
    println("This is MDS : " + meanDistanceSquared)
    
    // Computes the distance from the predicted value to the mean and sum them up
    val estimatedMinusMean = predictionAndLabel.map(x => (x._1 - mean) * (x._1 - mean)).sum()
    
    println("This is ESS : " + estimatedMinusMean)
    
    // R-Squared is derived from ESS / MDS
    val RSquared = estimatedMinusMean./(meanDistanceSquared)
    
    println("This is R-Squared : " + RSquared)
  }
}